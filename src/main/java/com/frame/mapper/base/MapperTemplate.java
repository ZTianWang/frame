...
public class MapperTemplate<T> {
	...
	public String updateColumns(Map<String, Object> parameter) {
		      ...
		      for(String column:columns){
		    	  if(i++!=0)sql.append(",");
		    	  sql.append(CamelNameUtils.camel2underscore(column)).append("=").append("#{").append(column).append("}");
	...
	public String updateColumnsByVersion(Map<String, Object> parameter) {
	      StringBuilder sql=new StringBuilder("update ").append(MyBatisHelper.getTableName((Class<?>)parameter.get("clz"))).append(" set ");
	      String [] columns=(String[]) parameter.get("columns");
	      int i=0;
	      for(String column:columns){
	      ...
	      sql.append(" where id=#{id} and version=#{where_version}");
	return sql.toString();
}
